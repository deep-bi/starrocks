...
//new
    ...
    // new
    template <typename From, typename To, bool check_overflow>
    static inline bool scale_up_wide(IntegerType<From> from_value, DecimalType<To> const& scale_factor,
                                DecimalType<To>* dec_value) {
        if constexpr (check_overflow) {
            auto overflow =
                    DecimalV3Arithmetics<To, check_overflow>::mul(static_cast<To>(from_value), scale_factor, dec_value);
    ...
    void scale_up_wide(int128_t from_value,
                   int128_t scale_factor,
                   int128_t& output_low,
                   int128_t& output_high) {
        // 1. Wyznacz znak wyniku
        bool negative = (from_value < 0) ^ (scale_factor < 0);

        // 2. Przejdź na wartości bezwzględne
        uint128_t a = abs(from_value);
        uint128_t b = abs(scale_factor);

        // 3. Policz pełny iloczyn 128x128 -> 256
        uint128_t low;
        uint128_t high;
        multiply_128x128_to_256(a, b, low, high);

        // 4. Jeśli wynik ma być ujemny,
        //    wykonaj negację 256-bitową (high, low)
        if (negative) {
            negate_256(high, low);
        }

        // 5. Zwróć obie połówki
